...
let
  ...
  mkAutoconfig =
    ...
    name: value:
    {
      status ? config.programs.firefox.preferencesStatus,
    }:
    ''
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    programs.firefox = {
      ...
      package = lib.mkDefault pkgs.firefox-esr;
      nativeMessagingHosts.packages = builtins.attrValues { inherit (pkgs) tridactyl-native; };
      preferencesStatus = lib.mkDefault "locked";
      autoConfig = lib.concatLines (
        [
          # (lib.replaceStrings [ "user_pref" ] [ "lockPref" ] (builtins.readFile ./betterfox.js))
