  ...
  userType = types.submodule (
    { name, config, ... }:
    {
      options = {
        ...
        enable = (mkEnableOption "home manager for the user") // {
          default = true;
        };
        modules = mkOption {
      ...
      config = {
        finalModules = lib.mkMerge [
          ...
          config.modules
          (lib.mkIf (home-manager.perUser != null) [ (home-manager.perUser config) ])
        ];
        ...
        finalConfigurations =
          let
            ...
            hostConfigs = lib.mapAttrs' (
              ...
              in
              lib.nameValuePair "${config.name}@${hostName}" (
                lib.mkIf enable (
          ...
          in
          lib.mkMerge [
            (lib.mkIf config.enable { ${config.name} = standaloneConfig; })
            hostConfigs
          ];
      };
    }
      ...
      type = types.submodule (
        { lib, ... }:
        {
          ...
          config.finalConfigurations = lib.mkMerge (
            lib.mapAttrsToList (_: value: value.finalConfigurations) cfg
          );
        }
      );
    };
